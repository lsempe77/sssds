---
title: "Spatial Analysis Severe Stigmatising Skin NTDs in Liberia - Years 2019-2021"
...
"vap" ="Annual Mean Evapotranspiration")
 ...
 map_final %>% 
      pivot_longer(all_of(bio), names_to = "variable", values_to = "values") %>%
  group_split(variable) %>% 
  map(~ggplot(.,aes(fill = values)) + 
       geom_sf() +
...
### Mean walking distance to health centre
...
```{r}
 ...
 map_final %>% 
  ggplot()+
  geom_sf(aes(fill = walk_liberia_mean)) +
  theme_void() + 
  scale_fill_viridis_c(name = "Mean distance walking (minutes)") + 
  theme (legend.position = "bottom")
